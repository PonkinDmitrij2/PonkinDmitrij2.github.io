...
var calculator = {
  result: 0,
  calc: function calc() {
    var values = [].concat(document.querySelectorAll('.calculator__field')).sort(function (a, b) {
...
var screen = {
  ...
  switch: function _switch(screenId) {
    ...
    document.querySelector('.page-header__layout').insertAdjacentHTML('beforeend', this.render(screenId));

    var toggleActive = function toggleActive(container, toggleClass, targetElemSelector) {
...
var calcBtnHandler = function calcBtnHandler() {
  var calcResultShow = document.querySelector('.calculator__calc-btn .toast__result');
